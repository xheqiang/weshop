...
<body>
<div class="gray_header"> <span class="title">更新栏目</span> <span class="reload"><a href="javascript:location.reload();">刷新</a></span> </div>
<form name="form" id="form" method="post" action="__URL__/doedit" onsubmit="return cfm_infoclass();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="form_table">
    		<tr>
			<td height="35" align="right">分类名称：</td>
			<td><input name="typename" type="text" id="classname" class="class_input" value="<?php echo ($list[typename]); ?>" />
		...
		<tr>
			...
			<td width="350">
		            <select name="f_id" id="parentid">
		            	...
		            	<?php }else{ ?>
		            		<option value="<?php echo ($list1[typeid]); ?>"><?php echo ($list1[typename]); ?></option>
		            		<option value="0">顶级分类</option>
		            	<?php } ?>	
		            	<?php if(is_array($lista)): $i = 0; $__LIST__ = $lista;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><option value="<?php echo ($vo[typeid]); ?>"><?php echo ($vo["typename"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			    </select>
		...
		<tr>
			...
			<td width="350">
			    <?php if($list[status]==2){ ?>
		            ...
		            <input type="radio" name="status" value="2" >未审核</input>&nbsp;&nbsp;
		            <input type="radio" name="status" value="1" checked="true">已审核</input>
		            <?php } ?>
            		</td>
			<td>&nbsp;</td>
		</tr>
	...
	<div class="subbtn_area">
		...
		<input name="typeid" type="hidden" value="<?php echo ($list[typeid]); ?>">
  </div>
</form>
<br /><br />
